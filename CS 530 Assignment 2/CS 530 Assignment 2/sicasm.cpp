...
int main(int argc, char* argv[]){
    ...
    printf(("Assembling file: " + fileName + "\n").c_str());
    SYMTAB = new TList();
    MODTAB = new TList();
    LITTAB = new TList();
    LTORGS = new TList();
    ...
    for(int i=0; i<SYMTAB->Count; i++) delete ((SIC_Symbol*)SYMTAB->Items[i]);
    for(int i=0; i<LITTAB->Count; i++) delete ((SIC_Literal*)LITTAB->Items[i]);
        ...
        if(StrPos(strLine.c_str(), "=C'") || StrPos(strLine.c_str(), "=X'")){
            ...
            if(!exists){
                ...
                if(token[0] == 'X'){
                    ...
                    if(val.Length() > 2) spaceReservedAfterLTORG += 3;
                    ...
                    String val(token);
                    val = val.SubString(1, val.Length()-1);
                    if(val.Length() > 1) spaceReservedAfterLTORG += 3;
                    else spaceReservedAfterLTORG += 3;
                }
            }
    ...
    if(op == -1){
        ...
        if(format == 1){
            ...
            if(StrPos(oper.c_str(), ",")){
                ...
                String o1 = oper.SubString(1, oper.Length() - o2.Length());
                int k=0;
                //is this a register?
            ...
            if(operand){
                ...
                //is there indexing?
                if(StrPos(operand, ",X")){
                    oper = oper.SubString(1, oper.Length()-2);
                    k |= BIT_X_3;
                }
                ...
                if(constant){
                  ...
                  while(1){
                    //are we good for program-counter relative addressing?
...
int GetOpcodeNum(char* str){
    ...
    for(i=0; i<NUM_OPCODES; i++)
        if(!strcmp(str, SIC_Opcodes[i].name)){ found = true; break; }
    if(found) return(i);
    else return(-1);
}
//---------------------------------------------------------------------------
...
int GetInstructionLength(char *instruction, char* operand){
    ...
    int size=0;
    if(instruction[0] == '+'){ formatFour = true; instruction++; }
